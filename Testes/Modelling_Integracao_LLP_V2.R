...
db.ALL$TU1_Urbanizacao = if_else( ( (db.ALL$CL_LOCALITY == 2 | db.ALL$CL_LOCALITY == 4) & (db.ALL$CL_LOCATION == 3 | db.ALL$CL_LOCATION == 4) &  db.ALL$POP_VarPop_C2001C2011 > 0) , 1, 0)
db.ALL[which(db.ALL$TU1_Urbanizacao == 1),c("DICOFRE18","FREG18_la.x")]

db.ALL$TU1_SubUrbanizacao = if_else( ( (db.ALL$CL_LOCALITY == 1) & (db.ALL$CL_LOCATION == 3 | db.ALL$CL_LOCATION == 4) &  db.ALL$POP_VarPop_C2001C2011 > 0 ) , 1, 0)
db.ALL[which(db.ALL$TU1_SubUrbanizacao == 1),c("DICOFRE18","FREG18_la.x")]

db.ALL$TU1_PeriUrbanizacao = if_else( ( (db.ALL$CL_LOCALITY == 1) & (db.ALL$CL_LOCATION == 2) &  db.ALL$POP_VarPop_C2001C2011 > 0 ) , 1, 0)
...
write_delim(db.all.store, paste(OUT.tables.path,"FREG_CAOP2018_TRANSF_data_CLASSIFICACAO5.txt", sep=""), quote_escape = "backslash")
