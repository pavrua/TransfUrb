...
OUT.MODELLING.FA.LOCALITY  = db.ALL %>% 
  ...
  summarise(
    LOCALITY_FA = round(mean(FA_LOCALITY, na.rm = TRUE),2)
  )

OUT.MODELLING.FA.LOCATION  = db.ALL %>% 
  group_by( CL_LOCATION ) %>% 
  summarise(
    LOCATION_FA = round(mean(FA_LOCATION, na.rm = TRUE),2)
  )
...
db.ALL$TU1_Metropolizacao = if_else( ( (db.ALL$CL_LOCALITY == 1 | db.ALL$CL_LOCALITY == 2 | db.ALL$CL_LOCALITY == 4) & (db.ALL$CL_LOCATION == 4) &  db.ALL$POP_VarPop_C2001C2011 > 0) , 1, 0)
nrow(db.ALL[which(db.ALL$TU1_Metropolizacao == 1),c("DICOFRE18","FREG18_la.x")])

db.ALL$TU1_SubUrbanizacao = if_else( (
  ( (db.ALL$CL_LOCALITY == 1) & (db.ALL$CL_LOCATION == 2) &  db.ALL$POP_VarPop_C2001C2011 > 0) |
    ( (db.ALL$CL_LOCALITY == 1) & (db.ALL$CL_LOCATION == 3 | db.ALL$CL_LOCATION == 2 | db.ALL$CL_LOCATION == 3) &  db.ALL$POP_VarPop_C2001C2011 > 0) |
    ( (db.ALL$CL_LOCALITY == 2) & (db.ALL$CL_LOCATION == 3) &  db.ALL$POP_VarPop_C2001C2011 > 0) )
  , 1, 0)
nrow(db.ALL[which(db.ALL$TU1_SubUrbanizacao == 1),c("DICOFRE18","FREG18_la.x")])

db.ALL$TU1_Urbanizacao = if_else( (
  ( (db.ALL$CL_LOCALITY == 2) & (db.ALL$CL_LOCATION == 1 | db.ALL$CL_LOCATION == 2) &  db.ALL$POP_VarPop_C2001C2011 > 0) |
  ( (db.ALL$CL_LOCALITY == 4) & (db.ALL$CL_LOCATION == 3) &  db.ALL$POP_VarPop_C2001C2011 > 0) )
  , 1, 0)
nrow(db.ALL[which(db.ALL$TU1_Urbanizacao == 1),c("DICOFRE18","FREG18_la.x")])

db.ALL$TU1_PeriUrbanizacao = if_else( ( (db.ALL$CL_LOCALITY == 3) & (db.ALL$CL_LOCATION == 2 | db.ALL$CL_LOCATION == 3) &  db.ALL$POP_VarPop_C2001C2011 > 0 ) , 1, 0)
nrow(db.ALL[which(db.ALL$TU1_PeriUrbanizacao == 1),c("DICOFRE18","FREG18_la.x")])

db.ALL$TU1_DeclinioUrbano = if_else( ( (db.ALL$CL_LOCALITY == 2 | db.ALL$CL_LOCALITY == 4) & (db.ALL$CL_LOCATION == 2 | db.ALL$CL_LOCATION == 3 | db.ALL$CL_LOCATION == 4) & db.ALL$POP_VarPop_C2001C2011 < 0 ) , 1, 0)
nrow(db.ALL[which(db.ALL$TU1_DeclinioUrbano == 1),c("DICOFRE18","FREG18_la.x")])

db.ALL$TU1_ContraUrbanizacao = if_else( ( (db.ALL$CL_LOCALITY == 3) & (db.ALL$CL_LOCATION == 1) & db.ALL$POP_VarPop_C2001C2011 > 0 ) , 1, 0)
nrow(db.ALL[which(db.ALL$TU1_ContraUrbanizacao == 1),c("DICOFRE18","FREG18_la.x")])

db.ALL$TU1_DeclinioRural = if_else( (db.ALL$CL_LOCALITY == 3 & (db.ALL$CL_LOCATION == 1) & db.ALL$POP_VarPop_C2001C2011 < 0 ) , 1, 0)
nrow(db.ALL[which(db.ALL$TU1_DeclinioRural == 1),c("DICOFRE18","FREG18_la.x")])


db.ALL$TU1Cod = "TUV1"

SEL_TU1 = c( "TU1_Metropolizacao",
             "TU1_SubUrbanizacao",
             "TU1_Urbanizacao",
             "TU1_PeriUrbanizacao",
             "TU1_DeclinioUrbano",
...
write_delim(db.all.store, paste(OUT.tables.path,"FREG_CAOP2018_TRANSF_data_CLASSIFICACAO7.txt", sep=""), quote_escape = "backslash")
